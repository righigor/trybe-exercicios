...
export async function createCityElement(cityInfo) {
  ...
  const previsaoButton = createElement('button', 'btn-previsao', 'Ver previsão');
  previsaoButton.addEventListener('click', () => btnObject(url));

  cityElement.appendChild(headingElement);
  cityElement.appendChild(infoContainer);
  cityElement.appendChild(previsaoButton);
...
export async function handleSearch(event) {
  ...
  cities.forEach(async (c) => {
    const weather = await getWeatherByCity(c.url);
    const info = { ...c, ...weather };
    const cityElement = await createCityElement(info);
